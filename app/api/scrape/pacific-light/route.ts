...
import { scrapePacificLightWebsitePrices } from "../../../../lib/scrapers/pacific-light";
...
export async function GET() {
  ...
  try {
    const supabase = createSupabaseAdmin();
    const prices = await scrapePacificLightWebsitePrices();

    const { error: deleteError } = await supabase
      .from("website_prices")
      .delete()
      .eq("company_id", "pacific-light");

    if (deleteError) {
      throw new Error(`Could not replace existing Pacific Light rows: ${deleteError.message}`);
    }
    ...
    if (insertError) {
      throw new Error(`Could not insert Pacific Light rows: ${insertError.message}`);
    }
